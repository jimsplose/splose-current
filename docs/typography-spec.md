...
## 5. Applying Typography in Code

### Two methods

1. **CSS utility class** — `className="text-display-lg"` — use for inline styling or inside DS components
2. **`<Text>` component** — `<Text variant="display/lg">` — use for page content where the semantic wrapper helps. Import from `@/components/ds`.
...
### What typography classes DON'T cover

Typography classes handle: font-family, font-size, font-weight, line-height, letter-spacing.

Keep these as separate Tailwind classes:
- **Color**: `text-text`, `text-text-secondary`, `text-primary`, `text-danger`
