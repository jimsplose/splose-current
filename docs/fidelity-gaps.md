...
- [x] **Dropdown item dividers** — Re-verified 2026-04-20: Localhost dropdown has only ONE divider (before destructive Archive item), matching prod behaviour. Earlier claim of dividers between all items was stale.
- [x] **Dropdown text color too light** — Re-verified 2026-04-20: Localhost dropdown items render at rgb(65, 69, 73), matching prod exact RGB. Gap was stale.
...
- [x] **Select button wrong color (purple vs mint/teal)** — Fixed 2026-04-20: `.selectBtn` updated to bg rgb(209,248,246), color rgb(0,0,0), 1px solid black border, 10px radius, 16px font. Measurement-verified: 100x42 (height +2px tolerance).
- [x] **Continue button size + styling** — Fixed 2026-04-20: Added `.continueFull` CSS module class + inline `{ width: 100%, height: 44, fontSize: 16 }` so disabled styling (`rgba(0,0,0,0.25)` bg, white text) reaches the AntD Button root. Full-width in sidebar card. Measurement-verified.
- [x] **Heading color** — Fixed 2026-04-20: `.pageTitle` color set to rgb(16, 24, 40). Measurement-verified.
- [x] **Appointment summary sidebar heading size** — Fixed 2026-04-20: `.sidebarTitle` font-size 16px, color rgb(17, 20, 26). Measurement-verified exact match.
- [x] **Location "Tasks" label styling inside card** — Fixed 2026-04-20: New `.locationName` class: 18px/500/rgb(16,24,40), block display. Measurement-verified.
- [x] **Book Now banner not rendering as banner** — Fixed 2026-04-20: Swapped DS `<Alert variant="info">` for semantic `<div className={styles.bookNowBanner}>` with bg rgb(201,228,252) / 1px rgb(130,80,255) border / 8px radius / 10px 12px padding. One-off banner per measurement-protocol 6.2. Measurement-verified exact match.
- [x] **Body background white vs peach** — Fixed 2026-04-20: `.page` bg overridden to rgb(250, 249, 245). Measurement-verified.
- [x] **Location placeholder image renders fire emoji** — Fixed 2026-04-20: Replaced 🔥 emoji with `<BankOutlined>` icon on neutral rgb(243,244,246) bg, 32px icon in rgb(107,114,128). Measurement-verified.
...
- [x] **Toolbar action buttons render as plain text** — Re-verified 2026-04-20: Pay (78x38), Email Invoice (143x38), Actions (104x38) all render as bordered `<Button variant="secondary">` with `1px solid rgb(65,69,73)`, bg white, 8px radius, 14px/400. Gap was already resolved in prior work.
...
- [x] **HICAPS sidebar card missing** — Fixed 2026-04-20: Added HICAPS card between Stripe and Note cards with gray banner (rgb(241,241,241)), HICAPS wordmark with "+" accents, description, and full-width outlined `<Button variant="secondary">Claim</Button>`. Measurement-verified: banner bg/height exact match.
- [x] **Stripe sidebar card layout completely wrong** — Fixed 2026-04-20: Reworked Stripe card with Card padding="none"; solid purple banner rgb(99,91,252) with lowercase "stripe" wordmark in white; description + purple `<a>` link "Connect to Stripe" at rgb(130,80,255). Measurement-verified: exact match to prod banner + link styling.
...
- [x] **Note textarea has placeholder, should be empty** — Fixed 2026-04-20: Removed placeholder attribute, added `fontFamily: 'inherit'` to override browser default monospace. Textarea now matches prod (empty + Inter font).
- [x] **Invoice card top gradient rainbow vs solid** — Fixed 2026-04-20: Replaced `linear-gradient(to right, #a855f7, #22c55e, #facc15)` with solid `rgb(134, 157, 252)`, bumped height 8→15px to match prod. Measurement-verified.
- [x] **"Additional Information" h4 too big/bold** — Fixed 2026-04-20: Changed from `<Text variant="heading/md">` (16px/600) to `<Text variant="body/sm">` with inline `fontSize:13, fontWeight:500`. Measurement-verified: 13px/500/rgb(65,69,73) exact match.
...
- [x] **Client alerts missing yellow left-border callout box** — Fixed 2026-04-20: Created new DS component `AlertCallout` (warning/danger/info variants) with 8px yellow left stripe via ::before, 1px gray border, 8px radius, 15px 15px 15px 23px padding. Wired into ClientDetailClient. Measurement-verified: all 14 properties exact match.
...
- [x] **Summary text font size + color** — Fixed 2026-04-20: Removed `text-body-md` class, inline `fontSize: 15` applied. Color already `rgb(65, 69, 73)`. Measurement-verified: 15px exact match.
