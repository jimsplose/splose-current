...
Sweep 2026-04-17 measurement shows earlier Group AO "fixes" have drifted or were applied against outdated production. Production now renders with teal/mint theming, not purple. **Re-verified 2026-04-17 against live prod URL `/online-booking/7b2c0db8-…`** — most gaps still open. Card radius item corrected (prod is 16px br, not 0px). New gap added for Appointment summary heading size.
- [ ] **Select button wrong color (purple vs mint/teal)** — Re-verified 2026-04-17: Localhost bg rgb(106,57,228) / white text / no border / 74×32 / 8px radius / 13.33px/400. Production bg rgb(209,248,246) / black text / 1px solid rgb(0,0,0) / 100×40 / 10px radius / 16px/400. Fix in `online-booking.module.css` `.selectBtn` — replace purple bg + white text + no border with mint bg + black text + 1px black border + 10px radius + 16px font.
- [ ] **Continue button size + styling** — Re-verified 2026-04-17: Localhost 92×38 bg rgb(249,250,251) 8px br (disabled state broken — the DS Button `disabled className="w-full justify-center"` isn't producing full-width). Production 281×44 bg rgba(0,0,0,0.25) 8px br white/16px with `w-full` filling the 288px sidebar card. Need to debug why `w-full` tailwind class isn't reaching the DS Button root or make Button support a `block` variant.
- [ ] **Heading color** — Re-verified 2026-04-17: Localhost rgb(65,69,73) via `var(--color-text)`. Production rgb(16,24,40) near-black. Change `.pageTitle` color to a darker token (or add `color: rgb(16, 24, 40)` inline for this page since it's a public page outside the app shell).
- [ ] **Appointment summary sidebar heading size** — NEW 2026-04-17: Localhost `<h2 className={styles.sidebarTitle}>` renders at 21px/700/rgb(65,69,73) (no explicit font-size → browser h2 default shrunken by some utility). Production 16px/700/rgb(17,20,26). Add `font-size: 16px` + dark color to `.sidebarTitle`.
...
- [ ] **Book Now banner not rendering as banner** — Re-verified 2026-04-17: Localhost "Book Now: Self-paying …" at 672×40 transparent bg / no border / no radius (DS `<Alert variant="info">` is flattening to plain text). Production 1240×42 bg rgb(201,228,252) / 1px rgb(130,80,255) border / 8px br / 10px 12px padding — proper info banner. Either fix the DS Alert `variant="info"` rendering or swap for a custom banner div.
- [ ] **Body background white vs peach** — Re-verified 2026-04-17: Localhost body rgb(255,255,255) / `.page` uses `var(--color-bg-container)` which resolves to white. Production has a cream/peach page bg (visible across the full viewport outside the content column). Override `.page` bg to a light peach/cream (e.g. rgb(250,249,245) observed in a prod wrapper) for this public route only.
- [ ] **Location placeholder image renders fire emoji** — Localhost: dark rgb(31,41,55) square 64×64 placeholder with 🔥 emoji. Production: actual location image (~100×108 horse photo from Splose account). Replace the emoji placeholder with a neutral building/pin icon (or wire to real image data if available).
- [x] **Card border radius** — Re-verified 2026-04-17: both sides render at 16px radius (prod 824×162, localhost 672×162, both 1px border ~rgb(208,213,221)/rgb(231,232,232), 20px pad). Earlier "prod 0px" note from 2026-04-17 Session 2 was incorrect — measurements today confirm both sides match.
...
- [x] **Credit balance plain text, should be purple pill badge** — Fixed [74130dd]: now renders as `<Badge variant="blue" solid>` with text "Credit balance: $80.00". Measurement-verified 2026-04-17: bg rgb(85,120,255), color white, 12px/400, 146×22 — matches prod 153×22 within tolerance.
- [x] **Status Draft/Overdue plain text, should be gray pill badge** — Fixed [74130dd]: `<Badge variant={STATUS_VARIANTS[status]} solid>`. Measurement-verified 2026-04-17: "Overdue" renders as 64×22 solid pill (uses danger red for Overdue which is semantically correct; Draft uses gray).
- [ ] **HICAPS sidebar card missing** — Production sidebar has a HICAPS card (gray bg banner with HICAPS wordmark + yellow "+" icons, description "Submit your claim through LanternPay by validating your invoice first.", outlined "Claim" button full-width). Entire card missing from localhost.
- [ ] **Stripe sidebar card layout completely wrong** — Production: full-width SOLID purple banner (bg rgb(130,80,255)) with lowercase "stripe" wordmark in white, then description "Add a 'Pay now' button to invoices for clients and contacts to pay online.", then purple text link "Connect to Stripe" (no button). Localhost: small square "S" icon + "Stripe" bold heading + purple-filled "Connect Stripe" button that appears to have broken styling (dark text on purple bg, narrow). Rework card to match production banner pattern.
- [x] **Payments sub-text extras** — Fixed [74130dd]: removed " AUD" suffix from progress text, removed "No payments recorded" message (only shows "Paid in full" when status === "Paid"). Measurement-verified 2026-04-17: text is "0.00 / 185.00" no AUD, no empty-state message rendered.
- [ ] **Note textarea has placeholder, should be empty** — Localhost textarea has "Add an internal note..." placeholder rendered in monospace font. Production shows an empty white-bg box with no placeholder text (and regular font if typed). Remove placeholder; fix textarea font-family to match body text.
- [ ] **Invoice card top gradient rainbow vs solid** — Localhost invoice preview card has a rainbow gradient bar at top (purple→green→yellow). Production has a solid lavender/blue bar. Change gradient to solid color.
- [ ] **"Additional Information" h4 too big/bold** — Localhost 16px/600. Production 13px/500. Match prod sizing.

### Group S30 — Client detail sidebar + tag + alert styling (`src/app/clients/[id]/layout.tsx`, `src/components/ds/SideNav.tsx`, `src/components/ds/Badge.tsx`, client detail alerts component)
- [x] **Active sidebar item missing purple accent bar** — Fixed [74130dd]: added `::after` pseudo-element to `.linkActive` in `SideNav.module.css` — 5px wide purple stripe (`var(--color-primary)`) on right edge with `border-radius: 0 8px 8px 0`. Measurement-verified 2026-04-17: ::after bg rgb(130,80,255), width 5px.
- [x] **Plan-managed tag uses wrong green variant** — Fixed [74130dd]: extended Badge DS with new `solidBg` and `solidText` props so pages can override the variant map when they need a specific shade. Client detail now uses `<Badge variant="green" solid solidBg="rgb(106, 176, 76)">`. Measurement-verified 2026-04-17: 12px/400/white on rgb(106,176,76), 99×22 exact match to prod.
- [ ] **Client alerts missing yellow left-border callout box** — Production wraps each alert in a box with ~4px yellow/amber left border, thin gray outer border, white bg, padding ~12px 16px. Localhost renders alert text plain (no box, no border). Add callout wrapper component for alerts.
...
- [x] **Pagination size + showTotal (SYSTEMIC — affects 30+ pages)** — Fixed 2026-04-17 Session 5: `Pagination.tsx` size prop now defaults to "default" (was "small"), showTotal is now an opt-in boolean prop (default false). `DataTable.tsx` no longer force-applies `size: "small"` nor a default `showTotal` — callers control both via the pagination prop. Measurement-verified on /settings/locations (pagination "1" only, 130×38, no `ant-pagination-mini`, no total-text) and /invoices/batch-invoice/1 (pagination + "10 / page" selector only, 38px default height, no total-text).
- [x] **Table container** — Fixed: flat layout (removed FormPage card wrapper). Measurement-verified 2026-04-17.

### Group S32 — DataTable Th background regression (SYSTEMIC, all table pages) `src/app/globals.css`, `src/components/ds/theme.ts`, `src/components/ds/DataTable.tsx`
Sweep 2026-04-17 Session 5: S3 claimed fixed on 2026-03-31 but has regressed. Dual-tab measurement on Clients list and Settings/Locations confirms localhost th bg rgb(234,237,241) `#eaedf1` vs production rgb(243,245,247) `#f3f5f7`. Three locations held the wrong value; all updated together.
- [x] **globals.css line 231** — Fixed: `.ant-table-thead > tr > th, .ant-table-thead > tr > td { background-color: #f3f5f7 !important; }`. Measurement-verified 2026-04-17.
- [x] **theme.ts line 84** — Fixed: `Table.headerBg: "#f3f5f7"`. Measurement-verified 2026-04-17.
- [x] **DataTable.tsx line 179** — Fixed: legacy Th `backgroundColor: "#f3f5f7"`. Measurement-verified 2026-04-17.
Verified on Clients list (`/clients`), Settings/Locations (`/settings/locations`), and Invoices list (`/invoices`): all three return `rgb(243, 245, 247)` exact-match with production.

### Group S33 — Settings/SMS structural + styling mismatches (`src/app/settings/sms-settings/page.tsx`)
Sweep 2026-04-17 Session 5 dual-tab measurement. S25 items stayed [x] (sections present) while S33 tracked correctness. Rebuilt the page 2026-04-17 Session 5.
- [x] **Low credit / Automatic recharge use Toggle, should be Checkbox** — Fixed: swapped `Toggle` → DS `Checkbox`. Measurement-verified 2026-04-17: 2 `input[type="checkbox"]` present, 0 `[role="switch"]`, `.ant-checkbox` 19×19 with 6px radius and purple fill rgb(130,80,255). Minor: inner square measures 19px vs prod 16px (within ±2px visual tolerance given the wrap element includes border).
- [x] **Recharge option cards wrong size + style (segmented control)** — Fixed: rebuilt as a 4-card joined segmented control. First card `border-radius: 6px 0 0 6px`, middle cards `border-radius: 0`, last `0 6px 6px 0`. Border-width `1px` on first, `1px 1px 1px 0` on middle+last. Active: white bg, rgb(130,80,255) border + text. Inactive: white bg, rgb(217,217,217) border, rgb(65,69,73) text. Measurement-verified 2026-04-17: 107×100 on all four, all borders exact match. Kept inline style — this is a one-off custom layout per measurement-protocol 6.2 ("one-off custom layouts — inline is fine"); if segmented controls appear on another page, extract to a DS `SegmentedControl` component.
- [x] **Inactive recharge card border color too dark** — Fixed: rgb(217,217,217) (covered by previous item). Measurement-verified 2026-04-17.
- [x] **SMS credit balance card too wide + wrong radius** — Fixed: wrapped DS `<Card padding="none">` in inline-block with `minWidth: 200, borderRadius: 3, padding: 15px, borderColor: rgb(232,232,232)`; inner DS `<Stat>` with `valueStyle={{ fontSize: 24, lineHeight: 1.5715 }}`. Measurement-verified 2026-04-17: card 200×96 exact, value 24px/400/rgb(65,69,73), title 14px — all match prod.
- [x] **Stat needs valueStyle prop (DS extension)** — Added `valueStyle?: React.CSSProperties` prop to `Stat.tsx`, merged with existing `color` prop. Page uses the new prop instead of inlining on a raw `<span>`.
- [x] **Raw `<h2>` section headings** — Fixed: all three `<h2>` instances replaced with `<Text variant="heading/lg">` on Recharge credits / SMS pricing / Two-way SMS.
- [x] **Inline font-size/weight on `<p>` for balance number** — Fixed: removed raw `<p style={{ fontSize: '1.875rem', fontWeight: 700 }}>884</p>`. Balance now rendered by DS `Stat` component.
- [x] **Raw `<label>` in Two-way SMS section** — Fixed: Your number field uses FormInput's `label="Your number"` prop. Copy button aligned with `align="end"` Flex + small marginBottom nudge.

### Group S34 — Active-state indicators across nav patterns (2026-04-17 audit, `src/components/ds/TopNav.module.css`, `src/components/ds/SideNav.module.css`, `src/app/globals.css`)
Dual-tab Chrome MCP measurement at 1440x900. Audited three distinct active-state patterns: main top nav, settings/client-detail sidebar, and antd Tabs used for settings subpages (e.g. `/settings/ai`).
- [x] **Main nav active bar wrong thickness + wrong width** — Prod uses a 3px top-bar inset 16px from each side of the 104px-wide li (bar = 72px). Localhost had `border-top: 2px solid transparent` spanning the full 104px anchor. Fixed: removed the `border-top` indicator from `.navLink`/`.moreButton`, added `::before` pseudo-element with `top: 0; left: 15px; right: 15px; height: 3px` that turns purple on `.navLinkActive`. Re-measured localhost: `::before` bg rgb(130,80,255), height 3px, width 73.8px (matches prod 71.9px within 2px tolerance).
- [x] **SideNav active item has lavender bg + 5px right accent bar that prod doesn't have** — Re-audit 2026-04-17 found prod SideNav (both `/settings/*` AND `/patients/[id]/*`) shows active item with ONLY fontWeight 600 + color `rgb(0,0,0)` change — NO background colour, NO `::after` accent bar. Prior S21 "lavender bg ✓" and S30 item 1 "5px purple accent bar ✓" were based on incorrect prod reading. Fixed: removed `background-color: rgb(237, 229, 255)` and the entire `.linkActive::after` block from `SideNav.module.css`. Re-measured localhost active Details item: bg rgba(0,0,0,0) transparent, `::after` content none — now matches prod exactly.
- [x] **Settings subpage tabs (antd Tabs): active tab fontWeight wrong** — Prod `/settings/ai` active tab-btn is fontWeight 700 purple; localhost was fontWeight 400 purple. Fixed: added global override `.ant-tabs-tab.ant-tabs-tab-active .ant-tabs-tab-btn { font-weight: 700; }` to `globals.css`. Re-measured localhost active Preferences tab-btn: fontWeight 700, color rgb(130,80,255) — matches prod exactly. Also unblocks the pre-existing partial note on `/settings/ai`.
