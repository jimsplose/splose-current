...
- [ ] **Status Draft/Overdue plain text, should be gray pill badge** — Production: span bg rgb(165,165,158), color white, 12px/400, padding 0 7px, border-radius 8px, ~45×22. Localhost: plain "Overdue" text. Use Badge (gray/muted solid variant) for the status token.
...
- [ ] **Payments sub-text extras** — Localhost shows "0.00 / 185.00 AUD" (with "AUD" suffix) + "No payments recorded" gray text below progress bar. Production shows "0.00 / 16.80" only (no AUD, no "No payments recorded" text). Remove AUD suffix and the empty-state text.
...
- [ ] **Active sidebar item missing purple accent bar** — Production active nav item ("Details") has a ~5-6px PURPLE VERTICAL BAR running along the right edge of the item (in addition to lavender bg). Localhost has lavender bg but no right-edge accent bar. Add a right border or absolutely-positioned accent stripe.
- [ ] **Plan-managed tag uses wrong green variant** — Localhost Badge for client tag renders subtle style (bg rgb(230,249,240) pale, color rgb(0,194,105) darker-green text). Production uses SOLID green fill (bg rgb(106,176,76), color white). The S21 variant="green" fix picked the wrong (subtle) green. Switch to Badge solid-green variant or add a "solid" prop for filled client tags.
