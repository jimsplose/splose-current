...
Sweep 2026-04-17 Session 5: S3 claimed fixed on 2026-03-31 but has regressed. Dual-tab measurement on Clients list and Settings/Locations confirms localhost th bg rgb(234,237,241) `#eaedf1` vs production rgb(243,245,247) `#f3f5f7`. Affects every table page — clients, contacts, invoices, payments, waitlist, products, all settings list pages, client sub-tabs, report tables. Three locations hold the wrong value and must all be updated together:
- [ ] **globals.css line 231** — `.ant-table-thead > tr > th, .ant-table-thead > tr > td { background-color: #eaedf1 !important; }` — change to `#f3f5f7`.
- [ ] **theme.ts line 84** — `Table: { headerBg: "#eaedf1" }` — change to `"#f3f5f7"`. (Card.headerBg on line 97 is already correct.)
- [ ] **DataTable.tsx line 179** — Legacy Th component hardcodes `backgroundColor: "#eaedf1"` — change to `"#f3f5f7"`.

### Group S33 — Settings/SMS structural + styling mismatches (`src/app/settings/sms-settings/page.tsx`)
Sweep 2026-04-17 Session 5 dual-tab measurement. S25 items were marked [x] because sections now exist, but the implementation uses the wrong controls and wrong option-card structure. Reopening as new group so S25 stays closed as "sections present" while S33 tracks the correctness gap.
- [ ] **Low credit / Automatic recharge use Toggle, should be Checkbox** — Production: 16×16 square checkbox with purple (rgb(130,80,255)) fill + white check + 4px radius, left of label. Localhost: 44×22 pill Toggle with white thumb. Swap `<Toggle>` for `<Checkbox>` (DS Checkbox exists at `src/components/ds/Checkbox.tsx`) on both the low-credit-balance and automatic-recharge rows.
- [ ] **Recharge option cards wrong size + style (segmented control)** — Production: 107×100 tall cards joined as a segmented control — first card `border-radius: 6px 0 0 6px`, middle cards `border-radius: 0` with only `border-width: 1px 1px 1px 0` (no left border), last card `border-radius: 0 6px 6px 0`. Active card: white bg, purple border + purple text. Inactive: white bg, rgb(217,217,217) border, rgb(65,69,73) text. Localhost: 119×38 separate rounded pills (`borderRadius: 8px`, `padding: 12px 20px`) with lavender bg rgba(130,80,255,0.1) when active. Rebuild as joined segment group — either a new `<SegmentedControl>` DS component or dedicated CSS on the option group. Remove the inline `backgroundColor` override on the active button.
- [ ] **Inactive recharge card border color too dark** — Production border rgb(217,217,217). Localhost border rgb(65,69,73) (reads as near-black). Soften to light gray.
- [ ] **SMS credit balance card too wide + wrong radius** — Production: 200×96 inline card, `border-radius: 3px`, border rgb(232,232,232), padding 15px. Localhost: 1051×103 full-width card, `border-radius: 12px`, border rgb(237,240,243), padding 16px 20px. Change `Card` to not stretch to container width (add `display: inline-block` or drop the `className="inline-block"` if already present — this is failing) and override `borderRadius: 3` + `padding: 15px`. Keep subtle border.
- [ ] **Raw `<h2>` section headings** — Lines 41, 75, 129 in `page.tsx` render `<h2 className="mb-4 text-heading-lg text-text">`. Swap for DS `<Text variant="heading/lg">` per DS component mandate.
- [ ] **Inline font-size/weight on `<p>` for balance number** — Line 34: `<p style={{ fontSize: '1.875rem', fontWeight: 700 }}>`. Replace with DS Text variant or extract as a DS `<Stat>` variant. (DS Stat component exists.)
- [ ] **Raw `<label>` in Two-way SMS section** — Line 135: `<label className="text-label-lg text-text mb-1 block">Your number</label>`. Use FormInput's `label` prop instead.
