...
| Filename | State | Match |
|---|---|---|
| (no reference screenshot) | Batch invoice detail — table with invoices, status badges, action buttons | partial — audit 2026-04-16: title 30px/700/green matches via FormPage PageHeader. But page is structurally incomplete: only 5 of 10 prod columns (missing To, Location, Practitioner, # of items, Issue date, Due date). Only "Export" btn vs prod "Mark as Sent"+"Email invoices". Summary text differs ("Created on..." vs prod "This batch contains N invoices totalling X AUD"). Badge style differs (outlined vs prod filled gray). Th bg rgb(234,237,241) vs prod rgb(243,245,247). No pagination. Invoice link fontWeight 500 vs prod 400. No row border-bottom (prod has 1px solid). DS Grade A. |
