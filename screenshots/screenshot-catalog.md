...
**Sweep 2026-04-17 (Session 7 — Active-state nav audit + fix)** — Targeted audit of three active-state patterns with dual-tab Chrome MCP + JS measurement at 1440x900. **S34 closed (3 items):** (1) **Main nav active bar** — prod 3px/72px inset 16px each side; localhost was 2px/104px full-width. Fixed `.navLink`/`.moreButton` in `TopNav.module.css` to use `::before` pseudo-element with top:0/left:15/right:15/height:3 and removed the full-width `border-top`. Re-measured: 3px tall, 73.8px wide (prod 71.9 within 2px). (2) **SideNav active item** — prod shows ONLY fontWeight 600 + color rgb(0,0,0) on active; localhost had lavender bg rgb(237,229,255) + 5px purple `::after` right-edge bar that DO NOT exist on prod. This reverses incorrect prior fixes (S21 "lavender ✓" and S30 item 1 "5px accent bar ✓") which were based on misreading prod. Fixed by removing `background-color` and the `.linkActive::after` block from `SideNav.module.css`. Re-measured: bg transparent, ::after content none — matches prod exactly. Affects BOTH settings sidebar AND client detail sidebar (same DS component). (3) **Settings subpage tabs (ant-tabs)** — prod active tab-btn fontWeight 700; localhost was 400. Fixed by adding `.ant-tabs-tab.ant-tabs-tab-active .ant-tabs-tab-btn { font-weight: 700 }` to `globals.css`. Re-measured `/settings/ai` Preferences tab: fontWeight 700 purple — matches prod. Build green. TSC clean.

**Fidelity 2026-04-17 (Session 6c — audit + commit parallel session's WIP)** — Parallel session auto-saved [74130dd] closing 5 S29/S30 items. Reviewed, TSC clean, measurement-verified all changes, tracking files updated. **Plan-managed badge** (S30 item 2): now SOLID rgb(106,176,76)/white/99×22 exact match (via new Badge `solidBg`/`solidText` props — DS extension, not bypass). **Active sidebar accent bar** (S30 item 1): `.linkActive::after` 5px purple stripe rgb(130,80,255) on right edge. **Invoice Credit balance pill** (S29 item 2): `Badge variant="blue" solid` — 146×22 purple solid matches prod 153×22 within tolerance. **Invoice Status pill** (S29 item 3): `Badge solid` — renders 64×22 filled pill (red for Overdue, gray for Draft via STATUS_VARIANTS map). **Payments sub-text** (S29 item 6): removed " AUD" suffix and "No payments recorded" text (only shows "Paid in full" when Paid). Client detail upgrades PARTIAL→closer-to-YES (only fontWeight 400-vs-600 on active sidebar item remains). Invoice detail closes 3 of 9 S29 items (Email invoice btn + HICAPS + Stripe + gradient + h4 + note-placeholder still open).

**Sweep 2026-04-17 (Session 6b — Post-deploy pagination correction)** — Parallel session verifying ship of S32/S33/S28. Confirmed S32 (th bg rgb(243,245,247)) holds on Clients/Locations/Invoices/Contacts/Payments/Waitlist/Products/Reports — systemic fix sticks. S33 (SMS rebuild) deployed successfully. **S28 correction discovered mid-sweep and committed [288e9bb]:** Session 5's `showTotal = false` default was wrong — production Clients list shows "1-10 of 642 items" + size changer. Reverted `Pagination.tsx` + `DataTable.tsx` to `showTotal` default true. Main list pages (Clients, Contacts, Invoices, Payments) now match prod. Settings list pages (Locations, Users, etc.) still show "1-6 of 6 items" which prod suppresses — tracked as follow-up (per-page opt out needed). Kept the `size="small"` removal (that part was universally correct). Also verified: Invoice detail credit balance pill now rgb(85,120,255) purple solid ✓ and status pill bordered ✓ (previously plain text). Invoice detail Email button still missing — S29 item open. Batch invoice summary now 14px/rgb(65,69,73) on BOTH sides — S26 final item closed. Client detail Plan-managed badge still subtle-green (S30 item 2 open).

**Sweep 2026-04-17 (Session 6 — Until-done full sweep)** — 9 batches dual-tab Chrome MCP + JS measurement at 1440x900. **Invoice detail (S29):** Re-measured — top TRR-006315 30px/700/green ✓ (S22 holds), Pay/Email/Actions white/bordered/8px/38h 14/400 ✓ (not "plain text" as previously claimed). Remaining gaps: Overdue/Draft pill localhost uses red-subtle rgb(254,242,243)/rgb(240,0,50) vs prod gray-SOLID rgb(165,165,158)/white/12px/400/8px-radius; Credit balance pill localhost green-subtle rgb(230,249,240)/rgb(0,194,105) vs prod purple-blue-SOLID rgb(85,120,255)/white; gradient band rainbow purple→green→yellow vs prod solid lavender; Stripe sidebar localhost square-S + filled button vs prod solid-purple banner + text link; HICAPS card missing entirely on localhost; Payments extra "AUD" suffix + "No payments recorded" text; Note textarea monospace + "Add an internal note..." placeholder vs prod empty; h4 "Additional Information" 16/600 vs prod 13/500. Stays PARTIAL. **Client detail (S30):** Lavender active-bg ✓, count badge style ✓. Remaining: purple vertical accent bar on right edge of active sidebar item still missing (clearly visible in prod zoom); active-item fontWeight 400 vs prod 600; Plan-managed pill subtle-green rgb(230,249,240)/rgb(0,194,105) vs prod solid rgb(106,176,76)/white. Stays PARTIAL. **Patient form view (S28) — MAJOR UPGRADE:** 24/700 client name ✓, 20/600 section title ✓, 14/500 field label ✓, "Not completed" italic ✓, file link blue rgb(0,0,255) ✓. Only 4 gaps now: "In progress" badge fontWeight 400 vs prod 700; missing centered white-card wrapper (content flows on raw page bg); "Not completed" color rgb(110,110,100) vs prod rgb(65,69,73); "No response" fontStyle italic on localhost vs normal on prod; logo 🦆 emoji vs prod image. Downgrade resolved 4/8 items. **Settings/SMS:** Noted — another session (5) identified structural gaps in toggle style, recharge card layout, credit balance card sizing; this sweep confirmed all 6 named sections now present (Two-way SMS ✓, Low credit balance ✓, Automatic recharge ✓), but agree with session 5 that styling still differs. Section heading weight localhost 18/700 vs prod 14/400 (localhost too heavy). Stays PARTIAL per S33. **Settings/AI Preferences REGRESSION:** Earlier 2026-04-16 fix with Wistia video + bordered card appears undone — now full-width flat sections, no Wistia, no bordered card. New regression flag. **Settings/AI block library tab:** "New" badge ✓, "+ New AI block" purple-filled ✓, "Your saved blocks" 16/600 heading ✓ — all 3 previously-listed items FIXED. Only remaining: top banner yellow-warning variant vs prod purple-info variant (icon style differs). **Settings/Details:** H1 30/700/green ✓, Save btn purple ✓. New gaps: `/settings` default route shows placeholder "All your settings in one place" vs prod which redirects/renders Details content; Business pill in email-signature tab is purple-filled/white on localhost vs gray-outlined on prod; dropdown values hydrate empty on localhost (Patient terminology, Currency code, Country, Currency symbol). **Calendar M1-M4:** M1 (Pay dropdown) localhost 2 items (Record payment, Send payment link) no-icons vs prod 6 items (Add payment, Add credit, Pay with credit, Stripe, Credit card, Terminal) with-icons. M2 (Create appointment) all 5 catalog items FIXED — title "Create appointment" ✓, side-by-side labels ✓, "2 waitlist matches" callout ✓, inline Time row ✓, required asterisks ✓. Localhost has extras (Scheduling conflict warning, Provider travel, Non-Labour Costs, Transport toggles) — not gaps. M3 (click-to-create popover) INVERTED — prod has 3 items, localhost 4 (extra "Availability"). M4 (appointment side panel) measured Archive btn prod RED rgb(240,0,50) outlined vs localhost GRAY outlined (catalog had this BACKWARDS — prod is red danger). ViewChangeLog prod centered vs localhost left. Status chevron "No status ▾" FIXED ✓. Still missing service-name title bar at top (prod shows "1:1 Consultation (1:1 Consultation)"). **Edit service (P1):** Same layout mismatch — localhost uses bold section headers (Pricing / Online booking / Online payment / Appointment notifications) vs prod flat form. Stays PARTIAL. **User details edit — MAJOR UPGRADE:** Full Account details title 30/700/green ✓, tabs Details/Availability/Body chart templates ✓, flat form with Title/First/Last name 3-col ✓, Gender ✓, Email ✓, DOB 3-col Day/Month/Year ✓, Phone numbers ✓, Professional title ✓, Groups ✓, Photo + Upload ✓, Timezone + Automatic toggle ✓, Account role ✓, Practitioner settings Locations checkboxes ✓. Catalog P1 "missing Title/Gender/DOB/Phone/Prof title/Photo" all RESOLVED. Upgrade PARTIAL→YES. **Dashboard new gap:** Widget heading banners — prod has 53px gray bg rgb(243,245,247) wrapping "Messages"/"Income"/"Incomplete progress notes"/"Recently submitted forms" headings, localhost has plain 22px heading text with no banner. Also Messages feed doesn't render inline images (prod shows GIFs/images), Income chart x-axis labels overlap, message input plain textarea vs prod rich editor. **Spot-checks YES confirmed:** Clients list (30/700/green h1 + 14/600 th), Calendar week (today btn 14/400 bordered, time label 10px, DOW 14/500/rgb(112,117,122)), Invoices list, Contacts list, Reports overview (cards 20/700), Settings/Locations, Settings/Users, Settings/Tags. **Notes edit/view:** Blocked this session — localhost canonical client has no note links. **Known remaining partials:** Login (illustration), Online booking (S31), Invoice detail (S29 — badges, gradient, Stripe/HICAPS), Client detail (S30 — accent bar, tag variant, fontWeight), Patient form (S28 — 4 items reduced from 8), Notes edit (S27 — documented), Calendar M1/M3/M4 (icons, extra Availability, service title bar, Archive color), Edit service (P1 layout), Settings/AI Preferences REGRESSION (lost Wistia/bordered card), Settings/SMS (S33 — per session 5), Dashboard (widget heading banners). **Upgrades this sweep:** User details edit (P1→YES), Patient form view (8→4 items), Calendar M2 (all 5 items fixed), Settings/AI block library (3 items fixed).

**Sweep 2026-04-17 (Session 5)** — Settings-focused sweep (parallel to another session) with dual-tab Chrome MCP + JS measurement. **Settings/SMS** (`/settings/sms-settings`): 3 previously-claimed fixes are structurally wrong. (1) Low-credit-balance + Automatic-recharge rows use 44×22 **Toggle** pill switches — production uses 16×16 **Checkbox** with purple fill + white check + 4px radius. (2) Recharge option cards render as 119×38 separate rounded pills with lavender bg rgba(130,80,255,0.1) when active — production is a joined **segmented control**: 107×100 tall cards, `border-radius: 6px 0 0 6px` first / `0` middle / `0 6px 6px 0` last, white bg with purple border + purple text when active. (3) SMS credit balance card is 1051×103 full-width with 12px radius — production is 200×96 inline card with 3px radius. Inactive recharge card border also wrong: localhost rgb(65,69,73) near-black vs prod rgb(217,217,217) light gray. New **S33** gap (7 items). Also 3 raw `<h2>` tags + inline font styles = DS Grade B. Stays PARTIAL. **Settings/Locations + Clients list** (ListPage re-verify): th background regression detected — localhost bg rgb(234,237,241) `#eaedf1` vs prod rgb(243,245,247) `#f3f5f7`. Same value on both tested pages so it's **SYSTEMIC across every table page**. S3 was marked fixed on 2026-03-31 but has regressed. Root cause: `globals.css:231` `!important` override, `theme.ts:84` `Table.headerBg`, and `DataTable.tsx:179` legacy Th — all three hold `#eaedf1` and must be updated together. New **S32** gap. Also on Locations: table cell names rendered as underlined links on localhost vs plain text on prod (minor — may be intentional navigation affordance). Pagination drift "1-6 of 6 items" present as expected (S28). Stays YES on all ListPages because th bg is a tight tolerance but flagged for fix. **Known remaining partials:** Login, Online booking (S31), Settings/SMS (S33), Settings/Integrations QB, Settings/AI block library, Client detail (S30), Invoice detail (S29), Batch invoice detail (S26 summary + pagination), Notes edit (S27). **Systemic:** S32 th bg regression (all tables), S28 pagination (30+ pages).

**Sweep 2026-04-17 (Session 4)** — Deep audit of Online booking public page (S31) dual-tab Chrome MCP + JS measurement at 1440x900. Live prod URL obtained via Settings → Online booking → Actions → View (`/online-booking/7b2c0db8-cb7b-40de-991e-631ecdb30cf0`). **Matches:** Location card (824×162 prod / 672×162 localhost — both 16px radius, 20px pad, 1px border ~rgb(208,213,221) vs rgb(231,232,232)), heading size 36px/600. **Mismatches (8 open):** Select btn MINT/black/bordered prod vs PURPLE/white/no-border localhost (drift from earlier Group AO fix — prod has shifted to mint theming). Continue btn 281×44/white/16px/600 vs 92×38/14px/400 (DS Button `disabled className="w-full justify-center"` failing to fill sidebar). Heading color rgb(16,24,40) vs rgb(65,69,73). Appointment summary h2 16px/700/rgb(17,20,26) vs 21px/700/rgb(65,69,73) **NEW**. Book Now banner 1240×42 bg rgb(201,228,252)/1px purple border vs 672×40 transparent plain text (DS Alert variant=info not rendering banner). Body bg pale peach (rgb(250,249,245) in prod wrapper) vs white. Location image horse photo vs 🔥 emoji placeholder. Location "Tasks" label 18px/500 stacked vs 14px/600 inline. **Corrected:** earlier "card radius prod 0px" claim was wrong — live measurement today shows both 16px. **Tooling finding:** Chrome MCP JS tool blocks outputs containing date strings or currency amounts (cookie/query filter false positive). Sanitize measurement snippets to avoid returning summary text verbatim. DS Grade B (~14 inline styles, mostly icon fontSize). Stays PARTIAL.
**Sweep 2026-04-17 (Session 3)** — Deep audit of Batch invoice detail (S26, oldest unverified partial) dual-tab Chrome MCP + JS measurement at 1440x900. **Batch invoice detail** (`/invoices/batch-invoice/[id]`): Major progress since original S26 audit (2026-04-16). **Fixed:** Title 30px/700/rgb(66,105,74) ✓, BATCH ref 18px/400/rgb(65,69,73) ✓, action buttons Mark as Sent 119x38 / Email invoices 125-126x38 white/1px rgb(65,69,73) border/8px br/14px/400 ✓, all 10 ths match (14px/600/rgb(65,69,73), bg rgb(243,245,247)) ✓, all 10 columns present ✓, status badges (Draft gray rgb(165,165,158) / Sent green rgb(180,235,100)) 22px h / 8px br / 12px/400 / 0 7px pad ✓, invoice link color rgb(130,80,255) ✓. **Remaining (2 gaps):** (1) Summary text 16px/rgb(0,0,0) localhost vs prod 15px/rgb(65,69,73). (2) **Systemic pagination drift** — DS `Pagination.tsx` hardcodes `size="small"` + `showTotal` so localhost renders 285x29 narrow right-aligned pill with "1-3 of 3 items" text, prod uses default-size (2515x38 full-width, no showTotal). Fixing DS Pagination will ripple across 30+ pages using it. **DS Grade B** (14 inline styles — 10 redundant Th `backgroundColor: TH_BG` should be Th default, 2 Button `borderColor/color` should be variant, 1 raw `<tr>` with inline border, BATCH span inline style). Stays PARTIAL.
**Sweep 2026-04-17 (Session 2)** — Batch 2 (2 more partials) dual-tab Chrome MCP + JS measurement at 1440x900. **Settings/Integrations**: Stripe/HICAPS/Tyro description copy exact-match ✓, Xero logo 100×100 ✓, Stripe Complete/Disconnect btns 233×38/106×38 ✓, Xero Connect 88×38 ✓. QB logo bounding box 292 vs prod 313 (minor). **UPGRADED NO→PARTIAL→YES**. Only remaining diff is systemic SideNav purple right-edge accent bar on active item, already covered in S30. **Online booking public page** (`/online-booking`): 7 gaps confirmed, earlier Group AO fixes have drifted or were applied against outdated production. Select btn MINT rgb(209,248,246)/black text/1px black border vs localhost PURPLE rgb(106,57,228)/white/no border — production theming has shifted to mint, localhost still on purple. Continue btn 281×44 vs 92×38. Card radius prod 0px vs localhost 16px. Tasks label prod 18px/500 stacked vs localhost 14px/600 inline. Book Now banner prod full-width 1240×49 vs localhost 672×40. Heading color prod rgb(16,24,40) vs localhost rgb(65,69,73). BG prod peach vs localhost white. Location image prod horse vs localhost fire emoji placeholder. New **S31 gap** (7 items). Stays PARTIAL.

**Sweep 2026-04-17** — Standard scope (2 partials) dual-tab Chrome MCP + JS measurement at 1440x900. **Invoice detail** (`/invoices/[id]`): S22 typography fixes still hold (status 26px/500 ✓, section labels 13px/700/rgb(65,69,73) ✓, Note/Payments 21px/500 ✓), but **new S29 gap** opens. Toolbar buttons Pay/Email invoice/Actions render as plain text on localhost — prod has bordered 98/139×38 buttons (1px solid rgb(65,69,73), 8px radius, 14px/400). Credit balance badge is plain text — prod is purple pill (bg rgb(85,120,255), white, 8px radius, 153×22). Status "Draft"/"Overdue" is plain text — prod is gray pill (bg rgb(165,165,158), white, 45×22). HICAPS sidebar card is MISSING entirely. Stripe sidebar card layout completely different — prod = full-width SOLID purple banner with lowercase "stripe" white text + "Connect to Stripe" purple text link; localhost = square "S" icon + "Stripe" bold heading + purple-filled "Connect Stripe" button (button looks broken/overflow). Payments sidebar has extra "AUD" suffix + "No payments recorded" text not on prod. Note textarea has mono "Add an internal note…" placeholder — prod empty box. Invoice card top gradient rainbow purple→green→yellow — prod solid lavender. h4 "Additional Information" 16px/600 vs prod 13px/500. **DOWNGRADED from YES to PARTIAL**. **Client detail** (`/clients/[id]`): S21 active lavender bg/count badges/collapse chevrons still hold, but **new S30 gap** opens. Active sidebar item missing purple vertical accent bar on right edge (prod has ~5-6px purple strip on right). Plan-managed tag uses subtle/outline variant (bg rgb(230,249,240), text rgb(0,194,105)) vs prod solid-green variant (bg rgb(106,176,76), white text) — S21 fix picked wrong Badge green variant. Client alerts have no yellow left-border callout box wrapping alert text — prod wraps each alert in a yellow-bordered container. Mailchimp SUBSCRIBED/ARCHIVED pattern differs (style + data). Stays PARTIAL. **Known remaining partials:** Login, Online booking, Settings/Integrations (QB logo/content), Settings/AI block library tab, Invoice detail (S29 — toolbar btns, badges, HICAPS, Stripe layout), Client detail (S30 — accent bar, tag variant, alert callout).

**Sweep 2026-04-16 (Session 4)** — Unaudited pages audit with dual-tab Chrome MCP + JS measurement at 1440x900. **Batch invoice detail** (`/invoices/batch-invoice/[id]`): First-ever audit. Title 30px/700/green matches (via FormPage). But structurally incomplete — only 5 of 10 prod columns, 1 of 3 action buttons, wrong summary text, wrong badge style, no pagination. Th bg rgb(234,237,241) vs prod rgb(243,245,247). New S26 gap (10 items). PARTIAL. **Notes edit page** (`/notes/[id]/edit`): First-ever dual-tab audit. Previous note IDs stale — found valid Draft note ID 35255497 via patient 5918810 ("a a"). Route mapping updated. Features match (SOAP sections, AI blocks, toolbar all present). But 8 styling gaps: "Save as final" btn GREEN vs prod PURPLE, Generate btn completely different style, title shows template abbrev not note name, client name 14px vs prod 18px, autosaved indicator badge vs text, Service label too bold/dark, missing AI sparkle header btn, transcript panel absent in split view. DOWNGRADED from YES to PARTIAL. New S27 gap (8 items). **Known remaining partials:** Login, Online booking, Settings/SMS, Settings/Integrations, Settings/AI block library tab, Client detail (sidebar), Invoice detail (regression), Batch invoice detail (structural), Notes edit (styling).
...
| Filename | State | Match |
|---|---|---|
| screencapture-acme-splose-settings-integrations-2026-03-17-18_34_17.png | Integrations list default | yes — sweep 2026-04-17: Stripe/HICAPS/Tyro description copy now exact-match prod, Xero logo 100×100 ✓, Stripe "Complete your profile" 233×38 + "Disconnect" 106×38 red ✓, Xero "Connect" 88×38 ✓, QB logo width 292 vs prod 313 (minor, 93%), img bounding boxes slightly wider but visible logo sizes match. Only remaining diff is the systemic SideNav purple right-edge accent bar on active item (covered in S30). |
...
| Filename | State | Match |
|---|---|---|
| Screenshot 2026-03-17 at 6.51.37 pm.png | Select a location step — default | partial — sweep 2026-04-17 (Session 4) re-audit against live prod `/online-booking/7b2c0db8-…`. **Matches now:** card radius both 16px (earlier note said prod 0px — incorrect), card padding 20px both, card border ~1px, heading font 36px/600 both. **Still mismatched:** Select btn prod MINT rgb(209,248,246)/black/1px black border/100×40/10px/16px vs localhost PURPLE rgb(106,57,228)/white/no border/74×32/8px/13.33px. Continue btn prod 281×44/white/16px/600 bg rgba(0,0,0,0.25) full-width disabled vs localhost 92×38/14px/400 bg rgb(249,250,251) — DS Button `disabled className="w-full justify-center"` not filling full width. Heading color prod rgb(16,24,40) vs localhost rgb(65,69,73). Book Now banner prod 1240×42 bg rgb(201,228,252)/1px rgb(130,80,255) border/8px br vs localhost 672×40 transparent (Alert variant=info flattening to plain text). Body bg prod pale peach (rgb(250,249,245) in wrapper) vs localhost white. Location image prod horse photo vs localhost 🔥 emoji 64×64 placeholder. **NEW:** Appointment summary h2 prod 16px/700/rgb(17,20,26) vs localhost 21px/700/rgb(65,69,73) (no explicit font-size in `.sidebarTitle`). 8 items remaining (1 corrected, 1 new). DS Grade B (~14 inline styles, mostly icon sizes). |
...
| Filename | State | Match |
|---|---|---|
| Screenshot 2026-03-17 at 7.26.58 pm.png | Edit note — AI-generated Subjective section with Accept button | partial — audit 2026-04-16 S4: SOAP sections + AI blocks structurally present. But: "Save as final" btn GREEN rgb(34,197,94) vs prod PURPLE rgb(130,80,255). Generate btn white/purple-border vs prod gray bg. Title shows "ST \| Note" template abbrev vs prod shows note name. Client name 14px/400 vs prod 18px/500. "Saved" green badge vs prod "Note has been autosaved" 13px text. |
| screencapture-acme-splose-notes-32681068-edit-2026-03-17-19_26_02.png | Progress note edit (variant 1) | partial — audit 2026-04-16 S4: all 5 SOAP sections, AI blocks, toolbar present (features match). Styling gaps: Save btn wrong color, Generate btn wrong style, title shows template not note name, transcript panel not shown in localhost split view, Service label 600/dark vs prod 400/gray. New S27 gap. |
| screencapture-acme-splose-notes-32681068-edit-2026-03-17-19_26_18.png | Progress note edit (variant 2) | partial — see S27, same styling gaps |
| screencapture-acme-splose-notes-32681068-edit-2026-03-17-19_26_27.png | Progress note edit (variant 3) | partial — see S27 |
| screencapture-acme-splose-notes-32681068-edit-2026-03-17-19_26_36.png | Progress note edit (variant 4) | partial — see S27 |
| screencapture-acme-splose-notes-32681068-edit-2026-03-17-19_27_15.png | Progress note edit (variant 5) | partial — see S27 |
| screencapture-acme-splose-notes-32681068-edit-2026-03-17-19_27_32.png | Progress note edit (variant 6) | partial — see S27 |
...
| Filename | State | Match |
|---|---|---|
| Screenshot 2026-03-11 at 11.10.50 am.png | Client detail — desktop, Details tab (General details, Account balance, Client alerts, Stripe, Mailchimp) | partial — sweep 2026-04-17: S21 active-bg/count-badges/chevrons hold, but 3 items open (S30). Missing purple vertical accent bar on right edge of active sidebar item ("Details"). Plan-managed tag uses subtle/outline variant (bg rgb(230,249,240), text rgb(0,194,105)) vs prod solid-green variant (bg rgb(106,176,76), white text). Client alerts have no yellow left-border callout box — prod wraps alert text in a yellow-bordered container. Mailchimp badge pattern also differs (subtle vs solid pill) but data values differ too. Typography still matches. |
...
| Filename | State | Match |
|---|---|---|
| Screenshot 2026-03-11 at 11.16.02 am.png | Invoice view — desktop, Overdue invoice PDF-style with Pay/Email/Actions buttons | partial — sweep 2026-04-17: S22 typography fixes hold (status 26px/500 ✓, section labels 13px/700 ✓, Note/Payments 21px/500 ✓) but new gaps found (S29). Toolbar buttons Pay/Email invoice/Actions render as plain text — prod has bordered 98/139×38 buttons (1px solid rgb(65,69,73), 8px radius). Credit balance is plain text — prod is purple pill badge rgb(85,120,255)/white. Status (Draft/Overdue) is plain text — prod is gray pill rgb(165,165,158)/white. HICAPS sidebar card is MISSING entirely. Stripe sidebar card uses completely different layout (square "S" + title + purple-filled button) vs prod full-width solid purple banner + "Connect to Stripe" text link. Payments shows extra "AUD" suffix + "No payments recorded" text not on prod. Note textarea has "Add an internal note…" placeholder in mono font — prod empty. Invoice card top gradient is rainbow (purple→green→yellow) — prod is solid lavender. h4 "Additional Information" 16px/600 vs prod 13px/500. |
...
## Batch Invoice Detail (`/invoices/batch-invoice/:id`)

| Filename | State | Match |
|---|---|---|
| (no reference screenshot) | Batch invoice detail — table with invoices, status badges, action buttons | partial — re-audit 2026-04-17: title 30px/700/green ✓, BATCH ref 18px/400/rgb(65,69,73) ✓, action buttons Mark as Sent 119x38 / Email invoices 125-126x38 (white/1px rgb(65,69,73) border/8px br/14px/400) ✓, all 10 ths 14px/600/rgb(65,69,73)/bg rgb(243,245,247) ✓, all 10 columns present ✓, status badges (Draft gray rgb(165,165,158), Sent green rgb(180,235,100)) 22px h / 8px br / 12px/400 / 0 7px pad ✓, invoice link color rgb(130,80,255) ✓. **Remaining S26 gaps (2 of original 8):** (1) Summary text 16px/rgb(0,0,0) localhost vs prod 15px/rgb(65,69,73). (2) Pagination uses DS hardcoded size="small" + showTotal "1-3 of 3 items" — prod uses default-size antd pagination full-width with NO showTotal text. Pagination is systemic (30+ pages use DS Pagination). DS Grade B (14 inline styles — 10 redundant `style={{backgroundColor: TH_BG}}` on Th, 2 inline Button border/color instead of variant, 1 raw `<tr>` with inline border, BATCH span inline style). |
